...
class MainPanel(tk.Frame):
    ...
    def _open_category_editor(self, tid: int, vals: tuple) -> None:
        ...
        # Transaction info
        tk.Label(dlg, text="Edit Category", bg=BG_PANEL,
                 fg=FG_PRIMARY, font=FONT_HEADING).pack(pady=(18, 4))
        info = tk.Frame(dlg, bg=BG_CARD, padx=14, pady=12)
        info.pack(fill="x", padx=24, pady=(0, 14))
        tk.Label(info, text=vals[1], bg=BG_CARD, fg=FG_PRIMARY,
                 font=FONT_HEADING, anchor="w").pack(fill="x")
        tk.Label(info, text=f"{vals[0]}  ·  {vals[4]}", bg=BG_CARD,
                 fg=FG_SECONDARY, font=FONT_BODY, anchor="w").pack(fill="x")

        # Category dropdown
        tk.Label(dlg, text="Category", bg=BG_PANEL, fg=FG_SECONDARY,
        ...
        btn = tk.Label(dlg, text="Save", bg=FG_BLUE, fg=FG_PRIMARY,
    ...
    def _open_category_editor(self, tid: int, vals: tuple) -> None:
        from models import CATEGORIES
        from views.theme import (BG_PANEL, BG_CARD, FG_PRIMARY, FG_SECONDARY,
                                  FG_BLUE, FONT_HEADING, FONT_SMALL, FONT_BODY)
