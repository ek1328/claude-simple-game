...
class ImportDialog(tk.Toplevel):
    ...
    def _build(self) -> None:
        # Header
        hdr = tk.Frame(self, bg=_th.BG_PANEL)
        ...
        tk.Label(hdr, text=f"Found {len(self.parsed)} transactions from {self.bank} — {stmt_label}",
                 bg=_th.BG_PANEL, fg=_th.FG_PRIMARY, font=FONT_HEADING).pack(side="left")
        tk.Label(hdr, text="Uncheck rows to skip. Set category per row.",
                 bg=_th.BG_PANEL, fg=_th.FG_SECONDARY, font=FONT_SMALL).pack(side="left", padx=16)

        # Select all / none
        ctrl = tk.Frame(self, bg=_th.BG_PANEL)
        ctrl.pack(fill="x", padx=24, pady=(0, 8))
        tk.Label(ctrl, text="Select:", bg=_th.BG_PANEL, fg=_th.FG_SECONDARY,
        ...
        # Scrollable table
        outer = tk.Frame(self, bg=_th.BG_PANEL)
        outer.pack(fill="both", expand=True, padx=24)

        canvas = tk.Canvas(outer, bg=_th.BG_PANEL, bd=0, highlightthickness=0)
        ...
        self._table = tk.Frame(canvas, bg=_th.BG_PANEL)
        ...
        # Summary bar
        tk.Frame(self, bg=_th.BG_CARD, height=1).pack(fill="x", pady=(8, 0))
        self._summary_frame = tk.Frame(self, bg=_th.BG_MAIN)
        self._summary_frame.pack(fill="x", padx=24, pady=(10, 0))
        self._build_summary()

        # Footer
        tk.Frame(self, bg=_th.BG_CARD, height=1).pack(fill="x", pady=(8, 0))
        footer = tk.Frame(self, bg=_th.BG_PANEL)
        footer.pack(fill="x", padx=24, pady=12)

        self._count_label = tk.Label(footer, text="", bg=_th.BG_PANEL,
                                     fg=_th.FG_SECONDARY, font=FONT_SMALL)
        self._count_label.pack(side="left")
        self._update_count()

        # Cancel
        btn_cancel = tk.Label(footer, text="Cancel", bg=_th.BG_CARD, fg=_th.FG_PRIMARY,
        ...
        btn_cancel.bind("<Leave>",    lambda e: btn_cancel.config(bg=_th.BG_CARD))

        # Total label next to Import button (updates with checkbox changes)
        self._footer_total_label = tk.Label(footer, text="", bg=_th.BG_PANEL,
                                            fg=_th.FG_GREEN, font=FONT_HEADING)
        self._footer_total_label.pack(side="right", padx=(0, 16))

        # Import
        btn_import = tk.Label(footer, text="↓ Import Selected", bg=_th.FG_BLUE, fg=_th.FG_PRIMARY,
                               font=FONT_HEADING, padx=14, pady=6, cursor="hand2")
        ...
        btn_import.bind("<Leave>",    lambda e: btn_import.config(bg=_th.FG_BLUE))
    ...
    def _build_table_header(self) -> None:
        ...
        hdr = tk.Frame(self._table, bg=_th.BG_HEADER)
        hdr.pack(fill="x")
        for col, w in zip(cols, widths):
            tk.Label(hdr, text=col, bg=_th.BG_HEADER, fg=_th.FG_SECONDARY,
                     font=FONT_SMALL, width=0, anchor="w",
                     padx=6, pady=6).pack(side="left", ipadx=w//8)
        tk.Frame(self._table, bg=_th.BG_CARD, height=1).pack(fill="x")
    ...
    def _build_table_rows(self) -> None:
        for i, txn in enumerate(self.parsed):
            ...
            bg = _th.BG_PANEL if i % 2 == 0 else _th.BG_ROW_ALT
    ...
    def _highlight_row(self, index: int, selected: bool) -> None:
        ...
        bg = _th.BG_SELECTED if selected else (_th.BG_PANEL if index % 2 == 0 else _th.BG_ROW_ALT)
        row.config(bg=bg)
    ...
    def _make_link(self, parent, text, command):
        lbl = tk.Label(parent, text=text, bg=_th.BG_PANEL, fg=_th.FG_BLUE,
    ...
    def _build_summary(self) -> None:
        ...
        stats = [
            ("Selected Total",  BusinessLogic.fmt_idr(total),   _th.FG_GREEN),
            ("Transactions",    str(count),                      _th.FG_BLUE),
            ("Largest Spend",   BusinessLogic.fmt_idr(highest),  _th.FG_RED),
            ("Average / txn",   BusinessLogic.fmt_idr(total / count) if count else "–", _th.FG_SECONDARY),
        ]

        for label, value, color in stats:
            box = tk.Frame(self._summary_frame, bg=_th.BG_PANEL, padx=14, pady=8)
            box.pack(side="left", padx=(0, 8))
            tk.Label(box, text=label, bg=_th.BG_PANEL, fg=_th.FG_SECONDARY,
                     font=FONT_SMALL).pack(anchor="w")
            tk.Label(box, text=value, bg=_th.BG_PANEL, fg=color,
    ...
    def _guess_category(self, merchant: str) -> str:
        ...
        m = merchant.lower()
        rules = [
            (["grab", "gojek", "uber", "pertamina", "toll", "parking", "parkir"], "⛽ Transport"),
            (["starbucks", "coffee", "kfc", "mcdonald", "pizza", "restaurant",
              "resto", "cafe", "gofood", "warung", "makan"], "🍽️ Food & Dining"),
            (["indomaret", "alfamart", "supermarket", "hypermart", "ranch market",
              "transmart", "hero", "carrefour"], "🛒 Groceries"),
            (["netflix", "spotify", "disney", "youtube", "icloud", "adobe",
              "canva", "notion", "figma"], "📱 Subscriptions"),
            (["shopee", "tokopedia", "lazada", "blibli", "zalora",
              "h&m", "zara", "uniqlo"], "🛍️ Shopping"),
            (["hotel", "traveloka", "airbnb", "airasia", "lion air",
              "garuda", "booking"], "✈️ Travel"),
            (["pln", "telkom", "indihome", "pam ", "water", "listrik"], "💡 Utilities"),
            (["apotik", "apotek", "farmasi", "klinik", "rs ", "rumah sakit",
              "halodoc", "alodokter", "gym"], "🏥 Health"),
            (["cgv", "cinema", "bioskop", "steam", "playstation"], "🎬 Entertainment"),
            (["ikea", "ace hardware", "ipl", "kost", "sewa"], "🏠 Housing"),
