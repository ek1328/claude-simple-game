...
class Sidebar(tk.Frame):
    ...
    def __init__(self, parent: tk.Widget, on_month_selected: Callable[[str], None]):
        super().__init__(parent, bg=_th.BG_SIDEBAR, width=240)
    ...
    def _build_empty_state(self) -> None:
        frame = tk.Frame(self._list_frame, bg=_th.BG_SIDEBAR)
        frame.pack(fill="both", expand=True, pady=40)
        tk.Label(frame, text="📂", bg=_th.BG_SIDEBAR, fg=_th.FG_SECONDARY,
                 font=("SF Pro Display" if __import__("platform").system() == "Darwin"
                       else "Segoe UI", 32)).pack()
        tk.Label(frame, text="No data yet", bg=_th.BG_SIDEBAR,
                 fg=_th.FG_PRIMARY, font=FONT_HEADING).pack(pady=(8, 4))
        tk.Label(frame, text="Upload a statement\nto get started",
                 bg=_th.BG_SIDEBAR, fg=_th.FG_SECONDARY, font=FONT_SMALL,
                 justify="center").pack()
    ...
    def _build(self) -> None:
        ...
        tk.Frame(self, bg=_th.BG_CARD, height=1).pack(fill="x", padx=12)
        self._build_scrollable_list()
        tk.Frame(self, bg=_th.BG_CARD, height=1).pack(fill="x")
        self._total_label = tk.Label(self, text="", bg=_th.BG_SIDEBAR,
                                     fg=_th.FG_SECONDARY, font=FONT_SMALL, pady=8)
        self._total_label.pack()

    def _build_search(self) -> None:
        frame = tk.Frame(self, bg=_th.BG_SIDEBAR, pady=10)
    ...
    def _build_section_label(self) -> None:
        tk.Label(self, text="MONTHLY OVERVIEW", bg=_th.BG_SIDEBAR,
                 fg=_th.FG_SECONDARY, font=FONT_SMALL
                 ).pack(anchor="w", padx=16, pady=(6, 4))

    def _build_scrollable_list(self) -> None:
        container = tk.Frame(self, bg=_th.BG_SIDEBAR)
        container.pack(fill="both", expand=True)

        self._canvas = tk.Canvas(container, bg=_th.BG_SIDEBAR, bd=0, highlightthickness=0)
        scrollbar = tk.Scrollbar(container, orient="vertical", command=self._canvas.yview)
        self._canvas.configure(yscrollcommand=scrollbar.set)
        scrollbar.pack(side="right", fill="y")
        self._canvas.pack(side="left", fill="both", expand=True)

        self._list_frame = tk.Frame(self._canvas, bg=_th.BG_SIDEBAR)
    ...
    def _build_month_row(self, mk: str, total: float, max_total: float, index: int) -> None:
        ...
        bg = _th.BG_SELECTED if is_selected else (_th.BG_CARD if index % 2 == 0 else _th.BG_SIDEBAR)
        ...
        tk.Label(inner, text=BusinessLogic.fmt_month(mk), bg=bg,
                 fg=_th.FG_PRIMARY, font=FONT_HEADING, anchor="w").pack(fill="x")
        ...
        bar.create_rectangle(0, 0, CANVAS_W, 6, fill=_th.BG_CARD, outline="")
        bar.create_rectangle(0, 0, fill_w, 6,
                             fill=_th.FG_GREEN if total < 5_000_000 else _th.FG_RED, outline="")

        tk.Label(bottom, text=BusinessLogic.fmt_idr(total), bg=bg,
                 fg=_th.FG_GREEN, font=FONT_SMALL, anchor="e").pack(side="right")
